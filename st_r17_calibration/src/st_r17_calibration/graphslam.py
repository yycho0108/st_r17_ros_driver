...
class GraphSlam3(object):
    def __init__(self, n_fix, n_dyn, lev=0.0):
        self._n_fix = n_fix
        self._n_dyn = n_dyn
        self._size = n_fix + n_dyn
        self._lev = lev
        self._graph = PoseGraph(size=self._size)

    def optimize(self, nodes, edges, **kwargs):
        # build graph + optimize
        [self._graph.add_node(n, i) for i, n in enumerate(nodes)]
        [self._graph.add_edge((ev,eo), i0, i1) for (i0, i1, ev, eo) in edges]
        res = self._graph.optimize(lev=self._lev, **kwargs)
        # marginalize all dynamic nodes + cleanup
        self._graph.del_node(i0=self._n_fix, i1=self._size)
        # TODO : call del_node first?
        self._graph.reset_edges()
